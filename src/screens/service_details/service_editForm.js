...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], serviceId, service, numfields, mapItem;

        service = row['service'];
        serviceId = row['service'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) serviceId = rslt.id;
                            
                            const mapPropKey = service.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "ServiceId", value: parseInt(serviceId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdateService(data, dropDownOptions);
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'service');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "Service is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
