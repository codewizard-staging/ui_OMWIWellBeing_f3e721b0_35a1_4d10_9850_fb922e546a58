...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, userId, numfields;

        const mapItems = MapItems;

        let user = row['user'];

        numfields = Helper.GetAllNumberFields(user);
        if (numfields.length > 0) Helper.UpdateNumberFields(user, numfields);


        // Add Or Update User
        rslt = await Support.AddOrUpdateUser(user, []);
        if (rslt.status) {
            userId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the userId and navigation entity if it is deos not exist
            ...
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    ...
                    rslt = await Support.AddOrUpdateUser(data);
