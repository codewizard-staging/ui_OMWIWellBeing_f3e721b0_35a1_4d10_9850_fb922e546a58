...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetServicesCount(query)
            .then(async (res) => {
                if (res.status) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetServiceSingle({ ServiceId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Services/edit/${id}`;
        if (type === 'view') _route = `/Services/view/${id}`;
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                            <IconButton
                                ...
                                onClick={() => NavigateTo("/Services/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
