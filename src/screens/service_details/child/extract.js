import ServiceJsonConfig from "config/service_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "service", expand: "", exclude: [], func: Support.AddOrUpdateService }
]

const FetchServiceInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(ServiceJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of ServiceJsonConfig[elm]) {
...
const FetchServiceDetails = async (serviceId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (serviceId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetServiceSingle(serviceId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetServiceSingle(serviceId);
            }
            
            if (rslt.status) {

                const service = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? service : service[source];
...
const Extract = async (serviceId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchServiceInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(serviceId)) {
                    await FetchServiceDetails(serviceId, enums).then(({ row, backRow }) => {
