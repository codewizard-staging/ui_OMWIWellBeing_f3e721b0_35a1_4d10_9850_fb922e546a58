...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, serviceId, numfields;

        const mapItems = MapItems;

        let service = row['service'];

        numfields = Helper.GetAllNumberFields(service);
        if (numfields.length > 0) Helper.UpdateNumberFields(service, numfields);


        // Add Or Update Service
        rslt = await Support.AddOrUpdateService(service, []);
        if (rslt.status) {
            serviceId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the serviceId and navigation entity if it is deos not exist
            let navItem = service.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "ServiceId", value: parseInt(serviceId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateService(data);
        ...
        global.AlertPopup("success", "Service is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
