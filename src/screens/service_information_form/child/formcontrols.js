        ...
        <Box sx={{ width: '100%' }}>
            <ValidatorForm ref={form} onSubmit={handleSubmit}>
                    <Box style={{ display: 'flex', width: '100%' }}>
                        {props.controls.service && <Box sx={{ width: "45%", margin: 2, boxShadow, borderRadius }}>
                            <RenderFormContols shadow={true} location="service" mode={props.mode} title={"Service"}
                                controls={props.controls.service} options={props.options} onInputChange={OnInputChange} />
                        </Box>}
                    </Box>
            </ValidatorForm>
        </Box>
